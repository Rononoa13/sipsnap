...
def test_returns_cached_image_without_calling_provider(tmp_path: Path,) -> None:
    ...
    service = DrinkImageService(
        ...
        cache_dir=tmp_path,
        client=httpx.Client(transport=httpx.MockTransport(lambda request: httpx.Response(200, content=b"fake-image-data")))
    )
...
def test_uses_next_provider_when_first_provider_has_no_image( tmp_path: Path,) -> None:
    class EmptyProvider:
        def find_image(self, drink_name: str) -> str | None:
            return None
